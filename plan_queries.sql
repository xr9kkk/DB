...
FROM Tournament t
         JOIN (SELECT tournament_id, COUNT(DISTINCT club_id) AS club_count
               FROM (SELECT tournament_id, club1_id AS club_id
                     FROM Match
                     ...
                     FROM Match
                     WHERE club2_id IS NOT NULL) clubs
               GROUP BY tournament_id) tc ON tc.tournament_id = t.tournament_id
WHERE tc.club_count = (SELECT MAX(club_count)
                       FROM (SELECT tournament_id, COUNT(DISTINCT club_id) AS club_count
                             FROM (SELECT tournament_id, club1_id AS club_id
                                   FROM Match
                                   WHERE club1_id IS NOT NULL
                                   ...
                                   SELECT tournament_id, club2_id AS club_id
                                   FROM Match
                                   WHERE club2_id IS NOT NULL) all_clubs
                             GROUP BY tournament_id) max_counts);
